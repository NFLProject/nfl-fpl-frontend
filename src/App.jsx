 ...
 * Option 1 fix: BASE_URL comes from env (Vercel: NEXT_PUBLIC_API_URL),
 * must be absolute and https in production.
 */

// --- CONFIG (read from env, normalize, and validate) ---
...
// Prefer env, else fallback to your known backend (https!)
const RAW_BASE =
...
// Guard against accidental http in production (mixed content -> CORS-like failure)
if (typeof window !== "undefined") {
  const isProdPage = window.location.protocol === "https:";
  const isHttpsApi = /^https:\/\//i.test(BASE_URL);
  if (isProdPage && !isHttpsApi) {
    // eslint-disable-next-line no-console
    console.error(
      "[API] Your page is https but API is not. Set NEXT_PUBLIC_API_URL to an https endpoint."
    );
  }
  // eslint-disable-next-line no-console
  console.info("[API] Using base:", BASE_URL || "(empty)");
}
...
// --- API helper with friendlier CORS/network errors ---
async function api(path, { method = "GET", body, userId } = {}) {
  ...
  try {
    const res = await fetch(url, {
      ...
      // credentials: "include", // <- enable ONLY if your backend uses cookies + CORS allow_credentials
    });

    // If the server responded but with an HTTP error, parse and throw
    if (!res.ok) {
    ...
    // Parse JSON response (or return null if no JSON)
    try {
    ...
    // This catch is hit on true network errors / CORS blocks / DNS / mixed-content
    const hint =
      "Network request failed. Likely causes:\n" +
      "• Wrong NEXT_PUBLIC_API_URL (empty, typo, or relative)\n" +
      "• CORS not allowing your frontend origin\n" +
      "• HTTPS mismatch (site is https, API is http)\n" +
      "• Backend is down or URL unreachable";
    // eslint-disable-next-line no-console
...
export default function App() {
  ...
  async function uploadStats() {
    ...
    try {
      ...
      alert(`Upload stats failed: ${e.message}`);
    }
  }
        ...
        >
          <div className="mb-4 flex gap-4 items-center">
            ...
            <input
              ...
              min={1}
              value={gw}
              onChange={(e) => setGw(Number(e.target.value) || 1)}
            />
            <Button
            ...
            <div>
              ...
              <div className="mt-4 grid grid-cols-3 gap-3 items-end">
                ...
                <div className="col-span-2 flex gap-3">
                  <Button
                    disabled={starters.length !== 9 || !captain || !vice || !userId}
                    onClick={saveLineup}
                  >
                    Save Lineup
                  </Button>
